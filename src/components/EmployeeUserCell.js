...
function EmployeeUserCell(props) {
  const { user } = props
  const [viewShow, setViewShow] = useState(false)
  const {deleteEmployeeUser} = useContext(JobsContext)
  return (
    <tr style={{ verticalAlign: "middle", tableLayout: "fixed", wordWrap: "break-word" }}>
      ...
      <td>
        ...
        <Button onClick={() => deleteEmployeeUser(user._id)} variant="outline-danger" className="me-2">
          Delete User
        </Button>
      </td>
...
export default EmployeeUserCell
