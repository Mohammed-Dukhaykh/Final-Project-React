...
function JobsRecommend() {
  const { jobs, profile } = useContext(JobsContext)
  if (!profile) return null
  const profileFound = profile.interesting.map(interest => interest._id)
  let jobFound
  if (profileFound) jobFound = jobs.filter(job => profileFound.find(interest => interest == job.jobField._id))
  jobFound.sort((a, b) => new Date(b.dateCreated) - new Date(a.dateCreated))
